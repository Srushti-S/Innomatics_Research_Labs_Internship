...
processor_e = {'Qualcomm Snapdragon 7c Gen 2': 1, 'Intel Pentium': 1, 'Intel Celeron': 1, 'AMD': 1, 'AMD Athlon': 1,
...
def data_transform(data):
    ...
    df_sample['Product'] = df_sample.Product.map(product_e)
    
    df_sample['Processor'] = df_sample.Processor.map(processor_e)
    
    encode_os = pickle.load(open(encode_os_path, 'rb'))
    
    df_sample['OS'] = encode_os.transform(df_sample['OS'])
    df_sample['RAM_type'] = df_sample.RAM_type.map(RAM_type)
    df_sample['HD_type'] = df_sample.HD_type.map(HD_type)
